...
command_exists nano && ! is_non_zero_string "${EDITOR}" && export EDITOR="nano"
# Use code if its installed (both Mac OSX and Linux)
command_exists code && ! is_non_zero_string "${EDITOR}" && export EDITOR="code --wait"
command_exists cursor && ! is_non_zero_string "${EDITOR}" && export EDITOR="cursor --wait"
...
if is_macos; then
  ...
  bindkey '^Xi' incremental-complete-word # shortcut: ctrl+i
  bindkey '^Xp' predict-on # shortcut: ctrl+p
  bindkey '^X^P' predict-off # shortcut: ctrl+shift+p
  ...
  if command_exists brew; then
    ...
    use_homebrew_installation_for "${HOMEBREW_PREFIX}/opt/wget"
    use_homebrew_installation_for "${HOMEBREW_PREFIX}/opt/gpg"
...
export PATH="/Users/rennokki/Library/Application Support/Herd/bin/":$PATH


# Herd injected PHP 8.4 configuration.
export HERD_PHP_84_INI_SCAN_DIR="/Users/rennokki/Library/Application Support/Herd/config/php/84/"
